          ...
          <Card className="p-4">
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="rounded-xl bg-primary/10 p-3 ring-1 ring-primary/10">
                <p className="text-xs text-primary">Optimized</p>
                <p className="mt-1 text-2xl font-semibold text-primary">{stats.optimized}</p>
              </div>
              <div className="rounded-xl bg-muted/60 p-3">
          ...
          <Card className="p-4">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-muted-foreground">Select a route to preview on map.</p>
              </div>
              <Link href="/plans/new">
            ...
            {isLoading ? (
              <div className="rounded-xl bg-muted/60 p-5 text-sm text-muted-foreground">Loading routes...</div>
            ) : plans.length === 0 ? (
              <div className="rounded-xl border border-dashed border-border p-5 text-center">
                <p className="font-medium">Belum ada rute hari ini.</p>
                <p className="mt-1 text-sm text-muted-foreground">Yuk buat pengiriman pertama.</p>
                <Link href="/plans/new">
                ...
                {plans.map((plan) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        'w-full rounded-xl border p-4 text-left transition-all hover:-translate-y-0.5 hover:shadow-soft',
                        isActive
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 line-clamp-2 text-sm text-muted-foreground">{plan.start_address}</p>
                        </div>
          ...
          <Card className="overflow-hidden p-0">
            ...
            <div className="p-4">
              {selectedPlan ? (
                ...
                <div className="flex h-[calc(100vh-250px)] min-h-[430px] items-center justify-center rounded-xl bg-muted/60 text-muted-foreground">
                  No route selected
                </div>
              )}
            </div>
            ...
            <Card className="p-5">
              ...
              {selectedPlan ? (
                <div className="mt-4 space-y-4">
                  <p className="text-sm leading-6 text-muted-foreground">{selectedPlan.start_address}</p>
                  <RouteSummary
                ...
                <p className="mt-4 text-sm text-muted-foreground">Choose a route from the left panel.</p>
              )}
            </Card>
            ...
            <Card className="p-5">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Operational stop order.</p>
                </div>
